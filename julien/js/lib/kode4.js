...
function init() {
	...
	ui.planetLabel = document.getElementById("planetLabel");  
    ...
    if (isMobil) {
		...
		theLogo.addEventListener('gesturechange', function (e) {
			e.preventDefault();
		}, false);		

		ui.myProfile.addEventListener('gesturechange', function (e) {
			e.preventDefault();
		}, false);		

		ui.myPlanets.addEventListener('gesturechange', function (e) {
			e.preventDefault();
		}, false);		

		ui.timelineZ.addEventListener('gesturechange', function (e) {
			e.preventDefault();
		}, false);		
    }
	...
	//ui.viewTimeline = document.getElementById('viewTimeline'); 	
	...
	var aLight = new THREE.AmbientLight( 0x3c3c3c ); 
	scene.add( aLight );	
	
	//mesh.light = new THREE.PointLight( 0xff4400, 1, 1000 );
	mesh.light = new THREE.PointLight( 0xffbb22, 1, 1000 );
	...
	//ui.viewTimeline.addEventListener('click', viewTimelineClick, false);    	
...
function theSun() {
	var geometry = new THREE.SphereBufferGeometry( 4, 24, 20 );
	//var material = new THREE.MeshBasicMaterial( { transparent: true, opacity: 0, blending: THREE.AdditiveBlending } );
	var material = new THREE.MeshStandardMaterial( { roughness: 0.3, metalness: 0.3, lightMapIntensity: 3 } );
	mesh.theSun = new THREE.Mesh( geometry, material );
	scene.add(mesh.theSun);
	mesh.theSun.visible = false; 
	mesh.theSun.name = 'theSun'; 
	//console.log(geometry.attributes.position); 

	var textureLoaderS = new THREE.TextureLoader(); 
	
	textureLoaderS.load( 'images/sun.jpg', function(tex) { 	
		mesh.theSun.material.map = mesh.theSun.material.lightMap = tex; 
		mesh.theSun.material.needsUpdate = true;
		mesh.theSun.visible = true; 
	});  	
	
	//var geometry = new THREE.CircleBufferGeometry( 24, 16 );
	var geometry2 = new THREE.CircleBufferGeometry( 12, 8 );
	...
	mesh.sunGlow = new THREE.Mesh( geometry2, material2 );
...
function thePlanets() {
	...
	for (var i = 0; i < 9; i++) {
		...
		var material = new THREE.MeshStandardMaterial( { transparent: true, opacity: 0, roughness: 0.5, metalness: 0.25, bumpScale: .1 } );
		mesh.planet[i] = new THREE.Mesh( geometry, material );
		
		mesh.planet[i].position.x = mesh.orbitGap * i + 12;
...
function theSatellite() {
	...
	function loadModel() {
		...
		//var material = new THREE.MeshStandardMaterial( { color: 0x88aaff, transparent: true, opacity: 0, roughness: 0.2, metalness: 0.6 } );
		var material = new THREE.MeshStandardMaterial( { color: 0xffcc66, transparent: true, opacity: 0, roughness: 0.1, metalness: 0.7 } );
		
		mesh.satellite = new THREE.Mesh(object.children[0].geometry, material);
		//mesh.satellite.scale.set(15, 15, 15); 
		mesh.satellite.scale.set(.86, .86, .86); 
		mesh.satellite.position.x = 8; 
		mesh.satellite.name = 'Kasini'; 
		
		scene.add(mesh.satellite);
	}
	...
	//loader.load( 'obj/iss2.obj', function ( obj ) {
	loader.load( 'obj/kasini.obj', function ( obj ) {
		object = obj;
	}, onProgress, onError );	
}
...
function theISS() {
	...
	function loadModel() {
		...
		mesh.ISS = new THREE.Mesh(object.children[0].geometry, material);
		mesh.ISS.scale.set(10, 10, 10); 
		//mesh.ISS.position.x = 8; 
		mesh.ISS.name = 'ISS'; 
...
function autoTypeIt(box) {
	...
	//if (box == ui.moiInfo) console.log(box);
	...
	switch (box) {
		case ui.moiInfo: 
			...
			break; 
		case ui.moiPlanets: 
			txt = ui.moiPlanets.innerHTML; 
			
			break; 
		//case ui.timelineEntry: 
			//txt = ui.timelineEntry.innerHTML; 
	...
	function typeIt() { 
		...
		} else if (mesh.currSelected == 2) {
			if (ui.viewTimeline) {
				ui.viewTimeline.removeEventListener('click', viewTimelineClick, false); 
				ui.viewTimeline = undefined;
				delete ui.viewTimeline; 		
			}
...
function exploreClick(e) {
	...
	var tweenB = new TWEEN.Tween( camera.position )
		...
		.onComplete(function() {
			...
			autoTypeIt(ui.moiInfo); 
			
			//ui.myMap.style.display = ui.myTimeline.style.display = "block"; 
			
			if (!isMobil) ui.kontainer.addEventListener('mousemove', onMouseMove2, false);
		})
		.start();	
	...
	var tweenE = new TWEEN.Tween( mesh.sunGlow.scale )
		...
		.onUpdate(function() {
			...
			mesh.planet[0].material.opacity = mesh.asteroids.material.opacity = mesh.asteroids2.material.opacity = mesh.asteroids3.material.opacity = mesh.satellite.material.opacity = mesh.ISS.material.opacity = this.x; 
		})
		.onComplete(function() {
			...
			mesh.satellite.material.transparent = mesh.ISS.material.transparent = false; 
			...
			mesh.asteroids.material.opacity = mesh.asteroids2.material.opacity = mesh.asteroids3.material.opacity = mesh.satellite.material.opacity = mesh.ISS.material.opacity = 1; 
		})
		.start();		
...
function onMouseMove2( event ) {
	...
	intersects = mesh.raycaster.intersectObjects([ mesh.planetGroup, mesh.theSun, mesh.satellite ], true);  
	...
	if (intersects.length > 0) {
		...
		var pName = mesh.intersected.name; 
		
		if (pName != '') {
			if (pName == 'Kasini') {
				pName = 'Cassini'; 
			} else if (pName == 'theSun') {
				pName = 'Sun'; 
			}
			
			ui.planetLabel.innerHTML = pName; 
			
			ui.planetLabel.style.top = clientY + 10 + 'px'; 
			ui.planetLabel.style.left = clientX + 10 + 'px'; 
			
			ui.planetLabel.style.display = "block"; 
			//ui.planetLabel.style.opacity = 1; 
		
			if (ui.planetLabel.style.opacity == 0) {
				var tweenOpac = new TWEEN.Tween({x: 1})
					.to({ x: 0 }, 3000)
					.delay(200)	
					.onUpdate(function() {
						ui.planetLabel.style.opacity = this.x; 
					})
					.onComplete(function() {
						ui.planetLabel.style.opacity = 0; 
						ui.planetLabel.style.display = "none"; 
					})
					.start();						
				}
		}
...
function viewTimelineClick(event) {	
	if (event) event.preventDefault(); 
	...
	if (win.timelineOpen) {
		...
		if (ui.timelineEntry.children.length == 0) {
			...
			for (var i = 0; i < tlLength; i++) {
				...
				//if (i == (tlLength - 1)) autoTypeIt(ui.timelineEntry); 
			}
		}
	}	
...
function playSoundFX(fxURL) {
	function createSoundFX(url) {
		var listener = new THREE.AudioListener();
		camera.add(listener);
		
		mesh.soundFX = new THREE.Audio(listener);
		
		var audioLoader = new THREE.AudioLoader(); 
			
		audioLoader.load(url, function(buffer) {
			mesh.soundFX.setBuffer(buffer);
			mesh.soundFX.setLoop(false);
			mesh.soundFX.setVolume(1.);
			mesh.soundFX.play();
	...
	if (mesh.soundFX) {
		...
		if (mesh.prevSelected == mesh.currSelected) {
			mesh.soundFX.play(); 
		} else {
			mesh.soundFX = undefined;
			delete mesh.soundFX; 
...
function kontainerClick(event) {
	...
	if (mesh.intersected) {
		...
		mesh.prevSelected = mesh.currSelected; 
		
		var fxURL = ''; 
		
		switch (mesh.intersected.name) {
			case 'theSun': 
				ui.moiPlanets.innerHTML = planetData.theSun.content; 
				mesh.currSelected = -1; 
				...
				break;
			case 'Kasini': 
				ui.moiPlanets.innerHTML = planetData.Cassini.content; 
				mesh.currSelected = 0; 
				
				fxURL = 'fx/Cassini.mp3'; 
...
function animate() { 
	...
	//	mesh.uniFs.alpha.value = mti2;	
					
		mesh.light.intensity = mti2 + 1; 	
		
		for (var i = 0; i < 9; i++) {
			var og = mesh.orbitGap * i + 12, 
				orbtX = orbtY = orbtZ = 12, 
				tme = 0; 
		...
		if (mesh.satellite) {
			tme = elapsed * .26; 
			mesh.satellite.position.set(Math.sin(tme) * 8, 0, Math.cos(tme) * 8); 
			mesh.satellite.rotation.set(Math.cos(tme), Math.sin(tme), Math.sin(tme)); 
		}		
		
		if (mesh.ISS) {
			tme = elapsed * .4; 
			mesh.ISS.position.set(Math.sin(tme) * 3 + mesh.planet[2].position.x, 0, Math.cos(tme) * -3 + mesh.planet[2].position.z); 
			mesh.ISS.rotation.set(Math.cos(tme), Math.sin(tme), Math.sin(tme)); 
		}
