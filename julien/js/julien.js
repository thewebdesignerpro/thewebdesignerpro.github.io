...
var camera, scene, renderer, controls, clock, group; 
var isMobil = false; 
var ui = {}, win = {}, mesh = {}; 
...
function init() {
	...
	ui.moiInfo.innerHTML = julienData.planet1.content; 
	
    clock = new THREE.Clock();	
	clock.autoStart = false; 	
	
//	mesh.panelGroup = new THREE.Group();
	...
	//window.addEventListener('click', addMesh, false);   
...
function thePlanets() {
	...
	for (var i = 0; i < 9; i++) {
		...
		var material = new THREE.MeshStandardMaterial( { transparent: true, opacity: 0, roughness: 0.5, metalness: 0.25, bumpScale: .18 } );
...
function exploreClick(e) {
	...
	var tweenF = new TWEEN.Tween( {x: -54})
		...
		.onComplete(function() {
			...
			/*
			mesh.panel1.position.set(posX, 5, -15);	
			//mesh.panel1.position.set(pos.x * .1, pos.y * .1, -14);	
			mesh.panel2.position.set(posX, 0, -15 );	
			//mesh.panel3.position.set(pos.x * .1, pos.y * -.1, -14);				
			mesh.panel3.position.set(posX, -5, -15);		

			mesh.panelLines[0].position.set(posX - 2.26, 6.47, -14.9 ); 
			mesh.panelLines[1].position.set(posX + 2.58, 6.31, -14.9 ); 
			mesh.panelLines[2].position.set(posX + 2.42, 3.47, -14.9 ); 
			mesh.panelLines[3].position.set(posX - 2.41, 3.63, -14.9 );
			
			mesh.panelLines[4].position.set(posX - 2.26, 2, -14.9 ); 
			mesh.panelLines[5].position.set(posX + 2.58, 1.84, -14.9 ); 
			mesh.panelLines[6].position.set(posX + 2.42, -2, -14.9 ); 
			mesh.panelLines[7].position.set(posX - 2.41, -1.84, -14.9 ); 
			
			mesh.panelLines[8].position.set(posX - 2.26, -3.47, -14.9 ); 
			mesh.panelLines[9].position.set(posX + 2.58, -3.63, -14.9 ); 
			mesh.panelLines[10].position.set(posX + 2.42, -6.47, -14.9 ); 
			mesh.panelLines[11].position.set(posX - 2.41, -6.31, -14.9 );				
			
			//mesh.panelGroup.visible = true; 
			mesh.panel1.visible = mesh.panel2.visible = mesh.panel3.visible = true; 
			
			for ( var i = 0; i < 12; i ++ ) {
				mesh.panelLines[i].visible = true; 
			}
			
			mesh.panelGrid1.position.set(posX, 5, -15.1); 
			mesh.panelGrid2.position.set(posX, 0, -15.1); 			
			mesh.panelGrid3.position.set(posX, -5, -15.1); 
			mesh.panelGrid1.visible = mesh.panelGrid2.visible = mesh.panelGrid3.visible = true; 
			*/
			//console.log(pos);			
		})
		.start();			
...
function onMouseMove2( event ) {
	...
	win.mouse.x = (event.clientX / win.width) * 2 - 1;
    win.mouse.y = -(event.clientY / win.height) * 2 + 1;    
	...
	if (mesh.intersected) {
		switch (mesh.intersected.name) {
			case 'planet1': 
				...
				break;
			case 'planet2': 
				ui.moiInfo.innerHTML = julienData.planet2.content; 
				
				break;
			case 'planet3': 
				ui.moiInfo.innerHTML = julienData.planet3.content; 
				
				break;
			case 'planet4': 
				ui.moiInfo.innerHTML = julienData.planet4.content; 
				
				break;
			case 'planet5': 
				ui.moiInfo.innerHTML = julienData.planet5.content; 
				
				break;
			case 'planet6': 
				ui.moiInfo.innerHTML = julienData.planet6.content; 
				
				break;
			case 'planet7': 
				ui.moiInfo.innerHTML = julienData.planet7.content; 
				
				break;
			case 'planet8': 
				ui.moiInfo.innerHTML = julienData.planet8.content; 
				
				break;
			case 'planet9': 
				ui.moiInfo.innerHTML = julienData.planet9.content; 
		...
		if (win.aboutOpen) {
			ui.myProfile.style.opacity = 0; 

			var tweenIt = new TWEEN.Tween( {x: 0})
				.to({ x: 1 }, 1000)
				.delay(100)	
				.onUpdate(function() {
					ui.myProfile.style.opacity = this.x; 
				})
				.onComplete(function() {
					ui.myProfile.style.opacity = 1; 
				})
				.start();					
		} else {
			ui.myProfile.classList.add('shown'); 
...
function aboutJMClick(event) {
	...
	if (win.aboutOpen) {
		...
		ui.aboutJM.innerHTML = '<img src="images/gear2.png" alt="">'; 
		win.aboutOpen = false; 		
	} else {
		ui.myProfile.classList.add('shown'); 
...
//	mesh.uniF1.time.value -= mti2;	
//	mesh.uniF1.alpha.value = mti2 * 2;	

//	mesh.uniF2.time.value += mti2 * .025;	
	//mesh.uniF2.alpha.value = mti2 * -2;	
	
//	mesh.uniF3.time.value += mti2 * .08;	
//	mesh.uniF3.alpha.value = mti2 * .1;	
