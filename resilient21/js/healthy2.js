...
var mouseX = mouseY = mouseZ = kr = 0, ww = window.innerWidth, wh = window.innerHeight, wwh = ww/2, whh = wh/2, entro = true,
	...
	renderer, camera, scene, spotL, controls, stats, lookAtScene = kon = kos = orthographicMode = false, zoom = 1.0, mergedMesh, icon1, icon2, icon3, icon4, icon5, icon6, icon7, icon8;
	...
	loader.load(
		...
		function (texture) {
			for ( var i = 0; i < dataLength; i++ ) {
				...
				material = new THREE.SpriteMaterial({map: icon6, fog: true, depthWrite: false, depthTest: false});
				...
				material = new THREE.SpriteMaterial({map: icon7, fog: true, depthWrite: false, depthTest: false});
				...
				material = new THREE.SpriteMaterial({map: icon8, fog: true, depthWrite: false, depthTest: false});
