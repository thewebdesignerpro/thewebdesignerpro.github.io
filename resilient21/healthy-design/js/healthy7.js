...
(function(){
// Fence object
var fencePoly = [[-106.46045032417464, 31.764792909271783 ], [ -106.46649360587249, 31.762053867926717 ], [ -106.46953698514479, 31.761053900451532 ], [ -106.46975436937852, 31.76366251125636 ], [ -106.46971089253177, 31.768010195931069 ], [ -106.46858049451635, 31.768792779172514 ], [ -106.46966741568502, 31.770357945655412 ], [ -106.46562406893754, 31.773488278621201 ], [ -106.4646675783091, 31.773444801774456 ], [ -106.46471105515586, 31.775618644111809 ], [ -106.46453714776887, 31.77605341257928 ], [ -106.46414585614815, 31.776357750506509 ], [ -106.46366761083392, 31.776662088433739 ], [ -106.46327631921321, 31.777009903207716 ], [ -106.46310241182621, 31.77757510221543 ], [ -106.45379836662234, 31.777531625368681 ], [ -106.45232015383293, 31.777009903207716 ], [ -106.45145061689799, 31.777009903207716 ], [ -106.45110280212401, 31.777009903207716 ], [ -106.4506245568098, 31.776575134740245 ], [ -106.45027674203583, 31.775836028345545 ], [ -106.45032021888257, 31.774966491410602 ], [ -106.45092889473703, 31.773357848080959 ], [ -106.45145061689799, 31.771966588985055 ], [ -106.45223320013945, 31.770923144663122 ], [ -106.45310273707439, 31.770836190969629 ], [ -106.45414618139631, 31.770662283582642 ], [ -106.45484181094426, 31.77022751511517 ], [ -106.45527657941173, 31.769792746647699 ], [ -106.45562439418572, 31.769097117099747 ], [ -106.4558852552662, 31.768097149624563 ], [ -106.45623307004017, 31.767010228455884 ], [ -106.45636350058041, 31.765749399900219 ], [ -106.45745042174909, 31.765662446206726 ], [ -106.45836343553079, 31.765618969359977 ], [ -106.45923297246573, 31.765401585126241 ], [ -106.46045032417464, 31.764792909271783]]
...
var	container = document.getElementById('container'), kontrols = document.getElementById("kontrols"), roads = document.getElementById("roads"), terrainM = document.getElementById("terrainM"), 
...
function theRoads() {
	...
	for (var i = 0; i < roadLength; i++) {
		...
		for (var j = 0; j < roadData.features[i].geometry.coordinates.length; j++) {
			...
			geometry.vertices.push(new THREE.Vector3(xyPos.x, 1.3, xyPos.z));
		}
...
// Draw the Polygons types - using Lines
...
/*
// Draw the Polygons types - using extrude geometry
function thePolygons() {
		...
		if (HealthyDesignData.features[i].geometry.type == 'Polygon') {
			for (var j = 0; j < HealthyDesignData.features[i].geometry.coordinates[0].length; j++) {
				...
				if (j == 0) {
					...
					polyShape.moveTo(xyPos.x, xyPos.z);
				} else {
					polyShape.lineTo(xyPos.x, xyPos.z);
				}
				//geometry.vertices.push(new THREE.Vector3(xyPos.x, 1.6, xyPos.z));
				//console.log('x: '+xyPos.x+' y: '+xyPos.z);
			}
		}
		
		polyShape.lineTo(firstX, firstY);
		polyShape.moveTo( 0,0 );
		polyShape.lineTo( 0, 100 );
		polyShape.lineTo( 100, 100);
		polyShape.lineTo( 100, 0 );
		polyShape.lineTo( 0, 0 );
		//var geometry = new THREE.ShapeGeometry(polyShape);
		var extrudeSettings = {amount: 3, bevelEnabled: true, bevelSegments: 2, bevelSize: 1, bevelThickness: 1};
		var geometry = new THREE.ExtrudeGeometry(polyShape, extrudeSettings);
...
// Draw the Multi Polygons types - using lines
...
/*
// Draw the Multi Polygons types - using extrude geometry
function theMultipolygons() {
	var material = new THREE.MeshBasicMaterial({color: 0x8822ee});
	
	for (var i = 0; i < dataLength; i++) {
		var multiPolyShape = new THREE.Shape();
		var firstX = firstY = 0;
		...
		if (HealthyDesignData.features[i].geometry.type == 'MultiPolygon') {
			for (var j = 0; j < HealthyDesignData.features[i].geometry.coordinates[0][0].length; j++) {
				...
				if (j == 0) {
					...
					multiPolyShape.moveTo(new THREE.Vector2(xyPos.x), new THREE.Vector2(xyPos.z));
				} else {
					multiPolyShape.lineTo(new THREE.Vector2(xyPos.x), new THREE.Vector2(xyPos.z));
				}
		...
		multiPolyShape.lineTo(new THREE.Vector2(firstX), new THREE.Vector2(firstY));

		//var geometry = new THREE.ShapeGeometry(multiPolyShape);
		var extrudeSettings = {amount: 3, bevelEnabled: true, bevelSegments: 2, bevelSize: 1, bevelThickness: 1};
		var geometry = new THREE.ExtrudeGeometry(multiPolyShape, extrudeSettings);
		var multipoly = new THREE.Mesh(geometry, material);
		multipolyGroup.add(multipoly);		
	}
...
function fencePolygon() {
	...
	for (var i = 0; i < fenceLength; i++) {
		var xyPos = LatLonToPixelXY(fencePoly[i][1], fencePoly[i][0]);	
	
		geometry.vertices.push(new THREE.Vector3(xyPos.x, 1, xyPos.z));
	}
	
	if (geometry.vertices.length > 0) {
		var fence = new THREE.Line(geometry, material);
		scene.add(fence);				
		var fence = new THREE.Line(geometry, material);
		fence.position.y += .2;
		scene.add(fence);				
