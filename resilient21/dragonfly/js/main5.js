...
var mouseX = mouseY = mouseZ = kr = 0, ww = window.innerWidth, wh = window.innerHeight, wwh = ww/2, whh = wh/2, entro = true,
	...
	pointclouds, threshold = 0.1, tTip, geometry, material, vertices, starS, starGeom, starMater, starS2, starGeom2, starMater2, arw2, wireframe, ii;
...
function drawLinksLines() {
	...
	var material = new THREE.LineBasicMaterial({color: 0x00c3ff, transparent: true, opacity: 0.45});
...
function animateFireflies(row, ffly) {
				...
				.onComplete(function() {
					...
					if (theNodes[currNode].type != "Path Node") {	
						for (var j = 0; j < ffly; j++) {	
							...
							var scale = (initFireflySize / (i - 10)) * 1.05;
	...
	//var tweenPassengers.length
		tweenPassengers[tweenPassengers.length - 1].onComplete(function() {
			//console.log(i);
			//console.log(thePassengers[0].scale.x);
