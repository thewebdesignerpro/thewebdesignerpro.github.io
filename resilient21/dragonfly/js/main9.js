...
var airportMesh, airplaneMesh, firefly, thePath, thePathway, thePathwayLength, pathLines, linkLines, fireflyCount = 10, currStream = 0;

// Arrays
var nodesOrbs = [], airplanes = [], airplaneOrbs = [], linksLines = [], thePassengers = [], tweenPassengers = [], tweenSet = [], 
	ii = [], setR = [], setG = [], initX1 = [], initY1 = [], initZ1 = [], tpwl = [];
...
function drawLinksLines() {
	...
	var material = new THREE.LineBasicMaterial({color: 0x00c3ff, transparent: true, opacity: 0.1});
...
function animateFireflies(row, ffly, callback) {
	...
	for (var i = 0; i < thePathwayLength; i++) {	
		if (thePathway[i + 1]) {
			...
			var node2 = parseInt(thePathway[i]);	
		}
	...
	for (var i = 0; i < thePathwayLength; i++) {	
		...
		if (node2 < dataNodesLength) {
			...
			tweenPassengers[i] = new TWEEN.Tween(thePassengers[ffly].position)
				...
				.onComplete(function() {
					ii[ffly] += 1;
					var currNode = parseInt(thePathway[ii[ffly]]);
						
					if (currNode < dataNodesLength) {
...
function commandTower() {
	...
	commandTowerMesh.position.y = 1.2;
