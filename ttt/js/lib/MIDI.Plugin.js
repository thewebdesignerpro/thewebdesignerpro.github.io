	...
	var audioLoader = function (urlList, index, bufferList, oncomplete) {
		...
		ctx.decodeAudioData(buffer, function (buffer) {
			...
			if (typeof(loader) !== "undefined") {
				...
				loader.message((index/87 * 100 >> 0)+"%<br>");
			}
			//console.log((index/87 * 100 >> 0)+"%");
			
			if (((index/87 * 100 >> 0)%6==0) && (cIndex < cTotalFrames-1)) {
				cXpos += cFrameWidth;
				cIndex += 1;
//				console.log('% '+percentComplete);
	...
	//document.getElementById('volumt').value = 1;
	//masterVolume = 0.1;
	//document.getElementById('volumt').addEventListener('change', function() {
	document.getElementById('volumt').addEventListener('input', function(e) {
...
//		gainNode2.connect(audioCtx.destination);		
	...
	//document.getElementById('volumt').addEventListener('onclick', function() {
		//return true;
	//});
