...
k10 = document.getElementById("k10"), k11 = document.getElementById("k11"), k11v = document.getElementById("k11v"), unt = document.getElementById("unt"), alrt = document.getElementById("alrt");
...
var domeTF=kon=kos=oldTx=false;

// metric
var metre = 35, feet = 3.28084; 
// meter to feet = 3.28084
var metric = true;

//var florHeight = -metre*2, projHeight=20;
var florHeight = -70, projHeight=20;
//var springL=metre*(themodels[curNum][3]*.26), podH=0;
var springL=metre*3.2, podH=0;
...
function init() {
	...
	//camera.position.copy(controls.center);
	...
	spotL0 = new THREE.SpotLight(0xffffff, 3, springL*4, Math.PI/4);
	//spotL0.position.set(0, 150, 0);
	spotL0.position.set(0, springL*2, 0);
...
/*	var geometry = new THREE.SphereBufferGeometry(rad+.8, 32, 32, 0, Math.PI*2, 0, Math.PI*.5);
	...
	//geom = new THREE.SphereGeometry(rad+.4, 32, 32, 0, Math.PI);
	geom = new THREE.SphereGeometry(rad+.4, 96, 96, 0, Math.PI);
	...
	domeB = new THREE.Mesh(geom, material2);
	...
	//if (metric) {
		k11.value = k11v.value = HH = rnd3(springL/metre);	
	//} else {
		//k11.value = k11v.value = HH = rnd3(springL/metre)*feet;	
	//}
	curNum = cNum;
	domeTF = true;
...
function projectoR(pNum) {
	...
	var material = new THREE.MeshPhongMaterial({color: 0x030303, shininess: 30});
	...
	if (themodels[cNum][10]=='box') {
		...
		if (metric) {
			...
			var geometry = new THREE.BoxGeometry(themodels[cNum][6]*metre*feet, k9.value*metre, themodels[cNum][7]*metre*feet);
		}
	} else {
		...
		if (metric) {
			...
			var geometry = new THREE.CylinderGeometry((themodels[cNum][6]*metre/2)*feet, (themodels[cNum][7]*metre/2)*feet, k9.value*metre, 32);
		}			
	}
	//console.log(HH);
...
//	angel=getDeg(themodels[cNum][8])/2;
	...
	var geometry = new THREE.CylinderGeometry(k8v.value*metre*.5, 1, H, 32, 1, true);
...
function alrter(tx) {
	...
	if (tx)	{
		...
		if (q<florHeight) {
			alrt.innerHTML = 'WARNING: WENT PAST FLOOR';
		//} else {
			//if (!oldTx) {
				//alrt.innerHTML = '';
				//oldTx = false;
...
/*	if (tx)	{
		alrt.innerHTML = oldTx = tx;
	} else {
		alrt.innerHTML = oldTx;
	}
		...
		console.log(alrt.style.opacity);
	}	*/
}
/*
function alrter2(tx) {
...
function uiX() {
	...
	var diatmp = themodels[curNum][3];
	if (!metric) diatmp *= feet;
	if (diatmp<k8v.value) {
		//k8.value = k8v.value = k8.max = themodels[curNum][3];
		k8.value = k8.max = themodels[curNum][3];
		alrter('Exceeded maximum dome size for this model');
		k8v.className = "red";
		oldTx = true;
		//console.log('max');
	} else {
		k8.max = themodels[curNum][3];
		k8v.className = "";
		oldTx = false;
		//console.log('max');
	}
	...
	if (!metric) {
		k6.innerHTML = rnd2(themodels[curNum][6]*feet)+' x '+rnd2(themodels[curNum][7]*feet); 	
		k9.value = k9v.innerHTML = k9.min = k9min.innerHTML = rnd2(themodels[curNum][4]*feet);
		k9.max = k9max.innerHTML = rnd2(themodels[curNum][5]*feet);		
		k8.min = 4.0 * feet;	
		k8.max *= feet;	
		k8.value = rnd2(k8.value * feet);
	} else {
		k8.min = 4.0;	
	}
...
function initUI() {
	...
	k8.value = k8v.value = themodels[curNum][3];
	uiX();
}
...
k8.addEventListener('input', function(e) {
	...
	k8v.value = this.value;
	
	k8Change();
	alrter();
	k8v.className = "";
}, false);
k8v.addEventListener('change', function(e) {
	//console.log('fire '+this.value);
	var disv;
	if (this.value<=0) {
		disv=k8v.value=.1;
	} else {
		disv=rnd2(this.value);		
	}
	if (disv<k8.min) {
		//k8.value = k8v.value = k8.min;
		oldTx = true;
		alrter('Min dome diameter is '+k8.min);
		//console.log('min'+disv+' k8min '+k8.min);
		//console.log('k8'+k8.value);
		//console.log('k8v'+k8v.value);
	} else	if (disv>k8.max) {
		//k8.value = k8v.value = k8.max;
		oldTx = true;
		alrter('Max dome diameter is '+k8.max);
		//console.log('max'+disv);
		//console.log('k8'+k8.value);
		//console.log('k8v'+k8v.value);		
	} else {
		k8.value = k8v.value = disv;
		alrter('');
...
k11.addEventListener('input', function(e) {
	...
	k11.value = k11v.value = HH = this.value;
	k11Change();
}, false);
k11v.addEventListener('change', function(e) {
	//k11.value = k11v.innerHTML = HH = this.value;
	//k11.value = k11v.value = HH = this.value;
	
	var disv=rnd2(this.value);
	if (disv<k11.min) {
		k11.value = k11v.value = HH = k11.min;
		//console.log('min'+disv+' k8min '+k8.min);
		//console.log('k8'+k8.value);
		//console.log('k8v'+k8v.value);
	} else	if (disv>k11.max) {
		k11.value = k11v.value = HH = k11.max;
		//console.log('max'+disv);
		//console.log('k8'+k8.value);
		//console.log('k8v'+k8v.value);		
	} else {
		k11.value = k11v.value = HH = disv;
		//console.log('pasok');
	}	
...
function unitS(s) {
	if (s==1) {
		if (!metric) {
			...
			k8.max /= feet;
			//k8.value /= feet;
			k8.value = k8v.value = rnd2(k8v.value / feet);
			//console.log('min '+k8.min+ 'max '+k8.max);
			k11.min /= feet;
			k11.max /= feet;
			k11.value /= feet;
			k11v.value = rnd2(k11v.value / feet);
			k6.innerHTML = themodels[curNum][6]+' x '+themodels[curNum][7]; 
			k9.min /= feet;
			k9min.innerHTML = rnd2(k9min.innerHTML / feet);
			k9.max /= feet;
			k9max.innerHTML = rnd2(k9max.innerHTML / feet);
			k9.value /= feet;
			k9v.innerHTML = rnd2(k9v.innerHTML / feet);
			k2.innerHTML = rnd2(k2.innerHTML / feet);
			//springL /= feet;
			metre *= feet;
			//console.log(springL);
			metric=true;
...
unit1.addEventListener('click', function(e) {
	unit1.className = "sel";
	unit2.className = "";
	unitS(1);
	e.preventDefault();
}, false);
unit2.addEventListener('click', function(e) {
	unit2.className = "sel";
	unit1.className = "";
	unitS(2);
	e.preventDefault();
...
function animate() {
	...
	//controls.center.set(0, domeF.position.y, 0);
	//controls.center.set(0, springL, 0);
	//controls.target = domeF.position;
...
function render() {
	camera.lookAt(scene.position);
	if (domeTF)	renderer.render(scene, camera);	
}
