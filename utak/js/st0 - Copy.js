...
function addSphere() {
	...
	loader.load( url2 + fileName + frmt, function(tx) { 	
		...
		const geometry = new THREE.IcosahedronGeometry(30, 30 );
		...
		const material = new THREE.ShaderMaterial({
			...
			fragmentShader: `
				...
				//    fragColor = vec4(colour, 1);
					...
					//vec3 lightDir = normalize(vec3(.5, -.5, .0));
					//vec3 lightDir = normalize(vec3(.9, -1.0, 1.1));
				...
				//    fragColor = vec4(vec3(colour - (diff * .8)), 1.);	
					fragColor = (gl_FragColor * .97) + vec4(vec3(colour - (diff * .9)), 1.);	
...
function onMouseMove( event ) {
   ...
   // if (event) event.preventDefault();
	...
	if (event.clientX) {
		...
		mouse.x = ( event.clientX - _.widthH ) / 10;   
		mouse.y = ( event.clientY - _.heightH ) / 10; 	
		
	} else {
		
		mouse.x = ( event.x - _.widthH ) / 10; 
		mouse.y = ( event.y - _.heightH ) / 10; 		
