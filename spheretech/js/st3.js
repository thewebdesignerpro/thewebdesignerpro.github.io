...
function addRocks() {
	...
	loader.load( 'models/islet.fbx', function ( object ) {
		...
		const geom = object.children[0].geometry,  
			  ...
			  posY = [.95, -.95, -.95, .95],  
			  //posZ = [],  
			  rotZ = [0, Math.PI/2, Math.PI/-2, Math.PI];
		...
		for ( let i = 0; i < 4; i++ ) {	
			...
			x.rocks[i].scale.set(2.7, 2.7, 2.7); 
			x.rocks[i].position.set(posX[i], posY[i], 2.4);
			x.rocks[i].rotation.set(0, 0, rotZ[i]);
