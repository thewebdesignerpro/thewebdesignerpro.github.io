...
function onWindowResize(event) {
	...
	/*if (uniforms1) {
		if (ww>wh) {
			uniforms1.resolution.value.x = ww;
			uniforms1.resolution.value.y = ww;		
		} else {
			uniforms1.resolution.value.x = wh;
			uniforms1.resolution.value.y = wh;
		}	
	}*/
	...
	var xx = wwh/6, yy = 73, zz = -200;
	
	if (wwh>whh) {
		if (ww>1366) {
			xx = wwh/6;
			if (ww>=2400) xx = wwh/8.5;			
		} else if (ww>800) {		
			xx = wwh/5;
		} else if (ww>640) {
			xx = wwh/3;
	...
	if (huid6) {
		huid1.position.set(xx, yy, zz);
		huid2.position.set(xx, 0, zz);
		huid3.position.set(xx, -yy, zz);
		huid4.position.set(-xx, yy, zz);
		huid5.position.set(-xx, 0, zz);	
		huid6.position.set(-xx, -yy, zz);
	}
