...
//	var url = 'aud/tmp/ctwi.'+ext;
	...
	analyser1 = new THREE.AudioAnalyser(sound, 128);
}
...
function viz(tx) {
	...
	loader.load('imj/shade/wat/rstones.jpg', function (tex) {
		...
		//tex.repeat.set(3, 3);		
		var geometry = new THREE.RingGeometry(.1, 150, 40, 50);
		...
		if (analyser1) {
			...
			for (var i=40; i<2040; i+=40) {
				...
				for (var j=0; j<40; j++) {
					...
					geomV.z = analyser1.data[imod]*-.07;
				}
			}			
