...
//    ui.kontainer.style.backgroundColor = '#d6053f';	
    ui.kontainer.style.backgroundColor = '#b836cc';	
	...
	win.ej[2] = .005	// cam rot mousex .005 or -.005	
...
//	const fogCol = 0xd900d3; 
	const fogCol = 0xb836cc; 
...
//    scene.add(camera);	
	...
	x.spotLight[0] = new THREE.SpotLight( 0xff10c1, 8000000, 5000, Math.PI/2.7, 1 );
	//x.spotLight[0] = new THREE.SpotLight( 0xff22cc, 8000000, 5000, Math.PI/5, 1 );
	//x.spotLight[0].position.set( 0, 175, -2000 );
//	x.spotLight[0].position.set( 750, 500, 750 );
	x.spotLight[0].position.set( 750, 400, 0 );
	//x.spotLight[0].position.set( 750, 200, 0 );
	x.spotLight[0].castShadow = true;
	//x.spotLight[0].shadow = new THREE.SpotLightShadow(camera);	
	x.spotLight[0].shadow.mapSize.width = 512;
	x.spotLight[0].shadow.mapSize.height = 512;
//	x.spotLight[0].shadow.camera.near = 10;
//	x.spotLight[0].shadow.camera.far = 5000;
//	x.spotLight[0].shadow.camera.fov = 60;
	x.spotLight[0].shadow.focus = 0.1; 
	//x.spotLight[0].shadowDarkness = 1.; 
	//x.spotLight[0].shadow.bias = .0001; 
	scene.add( x.spotLight[0] );	
	
	
	x.spotLight[1] = new THREE.SpotLight( 0x11cccc, 4000000, 5000, Math.PI/5, 1 ); 
	//x.spotLight[1] = new THREE.SpotLight( 0x11cccc, 8000000, 5000, Math.PI/2, 1 ); 
//	x.spotLight[1].position.set(-750, 400, -750 ); 
	x.spotLight[1].position.set(-750, 200, 0 ); 
	//x.spotLight[1].position.set(-750, 400, 0 ); 
	...
	//x.spotLight[1].shadow.focus = 0.1; 
...
*/	
	...
	//x.spotLight[0].castShadow = x.spotLight[1].castShadow = true; 
	x.spotLight[0].shadow.camera.near = x.spotLight[1].shadow.camera.near = 1; 
	x.spotLight[0].shadow.camera.far = x.spotLight[1].shadow.camera.far = 800; 
	//x.spotLight[0].shadow.camera.fov = x.spotLight[1].shadow.camera.fov = 50; 	
...
function addSky() {
	...
	loader.setPath( 'img/vaporwave/map/test2/' );
...
function addFloor() {
	...
	//let material = new THREE.MeshStandardMaterial( { color: kolor, roughness: 1, metalness: 0, transparent: true, opacity: .44 } );
	let material = new THREE.MeshLambertMaterial( { color: kolor, transparent: true, opacity: .44 } );
	let material2 = new THREE.ShadowMaterial( { color: 0x111111, transparent: true, opacity: .2 } );
...
function addHouse() {
	let meshCount = 0, 
		matr = [], 
		kolors = [0xffffff, 0xeeeeee, 0xffffff]; 
	
	let testMatr = []; 
	
	const loader = new OBJLoader();
	
	loader.load( 'obj/vaporw/house1.obj', function ( object ) {
		//console.log(object);
		...
		object.traverse( function ( child ) {
			if ( child.isMesh ) {
				...
				if (meshCount == 2) child.castShadow = true; 
				//if (meshCount == 1) 
		...
		for ( let i = 0; i < meshCount; i++ ) {	
			if (i != 2) {
				//matr[i] = new THREE.MeshPhongMaterial( { color: kolors[i], shininess: 80, transparent: true, opacity: .8 } );
				matr[i] = new THREE.MeshBasicMaterial( { color: kolors[i], transparent: true, opacity: .7 } );
			} else {
				matr[i] = new THREE.MeshStandardMaterial( { color: kolors[i], roughness: .9, metalness: 0 } );
			}
				
			x.house.children[i].material = matr[i]; 		
		}
		
		x.house.children[0].material.envMap = x.skybox; 
		x.house.children[1].material.envMap = x.skybox; 
		//x.house.children[2].material.roughness = .5; 
		
		//x.house.children[2].material.transparent = false; 
	...
	/*	testMatr[0] = new THREE.MeshStandardMaterial( { color: 0x111111, roughness: 0, metalness: .25, envMap: x.skybox } );	// main frame
		...
		*/
		let tempmat = new THREE.ShadowMaterial( { color: 0x111111, transparent: true, opacity: .2 } );	
		
		//x.car.children[0].material = testMatr; 
		//x.car2.children[0].material = testMatr; 
		//console.log(x.car.children[0].material[0]); 
		
		x.winShadow = new THREE.Mesh( object.children[1].geometry, tempmat ); 
		x.winShadow.castShadow = false; 
		x.winShadow.receiveShadow = true; 

		x.houseShadow = new THREE.Mesh( object.children[2].geometry, tempmat ); 
		x.houseShadow.castShadow = false; 
		x.houseShadow.receiveShadow = true; 
		
	/*	const geom = new THREE.BoxGeometry( 138, 2, 104 ); 
		const mater = new THREE.MeshStandardMaterial( {color: 0xffffff} ); 
		x.houseBase = new THREE.Mesh( geom, mater ); 
		x.houseBase.castShadow = true; 
		x.houseBase.receiveShadow = true; */
		
		//x.car3 = new THREE.Mesh( object.children[0].geometry, tempmat ); 
		
//		x.car2 = new THREE.Mesh( object.children[0].geometry, testMatr ); 
		
		x.house.scale.set(100, 100, 100); 
		
		x.winShadow.scale.set(100, 100, 100); 
		x.houseShadow.scale.set(100, 100, 100); 
		//x.car3.scale.set(10.02, 10.02, 10.02); 
		
//		x.car2.scale.set(10, 10, 10); 
		
		x.house.position.set(0, florY+0, 0); 
		x.house.rotation.set(0, Math.PI/2, 0); 
		
		x.winShadow.position.set(.1, florY+.1, .1); 
		x.winShadow.rotation.set(0, Math.PI/2, 0); 
		x.houseShadow.position.set(.1, florY+.1, .1); 
		x.houseShadow.rotation.set(0, Math.PI/2, 0); 
...
//		x.car2.rotation.set(0, 0, Math.PI); 
	...
	//	x.car2.receiveShadow = true; 		
		...
		grups[0].add( x.house );		
		grups[0].add( x.winShadow );		
		grups[0].add( x.houseShadow );		
		//grups[0].add( x.houseBase );		
...
function addBlinds() {
	const geometry = new THREE.PlaneGeometry( 102, 15 );
	const geometry2 = new THREE.PlaneGeometry( 35, 13 );
	const geometry3 = new THREE.PlaneGeometry( 53.5, 14 );
	
	//const material = new THREE.MeshPhongMaterial( {color: 0xd4bbaa, emissive: 0xd4bbaa, shininess: 8 } );
	const material = new THREE.MeshBasicMaterial( { color: 0xffffff } );
	const material2 = new THREE.MeshBasicMaterial( { color: 0xffffff } );
	const material3 = new THREE.MeshBasicMaterial( { color: 0xffffff } );
	
	x.blinds = new THREE.Mesh( geometry, material );
	x.blinds.position.set(-11.6, 8.8, -4.6);

	x.blinds2 = new THREE.Mesh( geometry2, material2 );
	x.blinds2.position.set(27, 27, 20);

	x.blinds3 = new THREE.Mesh( geometry3, material3 );
	x.blinds3.position.set(10.3, 27, -5.8);
	x.blinds3.rotation.set(0, Math.PI/-2, 0);
	
	grups[0].add( x.blinds );	
	grups[0].add( x.blinds2 );	
	grups[0].add( x.blinds3 );	
	
	let load1 = new THREE.TextureLoader(),  
		load2 = new THREE.TextureLoader(),  
		load3 = new THREE.TextureLoader(); 
		
	load1.load( 'img/vaporwave/blinds1.jpg', function(tx) { 
		tx.wrapS = tx.wrapT = THREE.RepeatWrapping;    
		//tx.wrapS = tx.wrapT = THREE.MirroredRepeatWrapping;    
		tx.repeat.set(5, 1);    		
		
		//for ( let j = 0; j < 1; j++ ) {	
			x.blinds.material.map = tx; 
			//x.blinds.material.emissiveMap = tx; 
			//x.car.material.alphaMap = tx; 
			
			x.blinds.material.needsUpdate = true; 
		//}
	}); 		

	load2.load( 'img/vaporwave/blinds1.jpg', function(tx2) { 
		tx2.wrapS = tx2.wrapT = THREE.RepeatWrapping;    
		tx2.repeat.set(1.7, 1);    		
		
		//for ( let j = 0; j < 1; j++ ) {	
			x.blinds2.material.map = tx2; 
			
			x.blinds2.material.needsUpdate = true; 
		//}
	}); 		

	load3.load( 'img/vaporwave/blinds1.jpg', function(tx3) { 
		tx3.wrapS = tx3.wrapT = THREE.RepeatWrapping;    
		tx3.repeat.set(3, 1);    		
		
		//for ( let j = 0; j < 1; j++ ) {	
			x.blinds3.material.map = tx3; 
			
			x.blinds3.material.needsUpdate = true; 
		//}
	}); 		
...
function addAircon() {
	...
	loader.load( 'obj/aircon/aircon1.obj', function ( object ) {
		//console.log(object);
		...
		object.traverse( function ( child ) {
			if ( child.isMesh ) {
				...
				//child.geometry.scale.set(.62, .62, .62); 
				child.geometry.computeVertexNormals();		
				child.geometry.computeBoundingBox();		
		...
		matr = new THREE.MeshStandardMaterial( { color: kolors[0], roughness: .5, metalness: .1 } );
		let tempmat = new THREE.ShadowMaterial( { color: 0x111111, transparent: true, opacity: .2 } );	
		
		for ( let i = 0; i < 4; i++ ) {	
			//x.aircon[i] = object; 
			x.aircon[i] = new THREE.Mesh( object.children[0].geometry, matr ); 
			x.acShadow[i] = new THREE.Mesh( object.children[0].geometry, tempmat ); 
			
			x.aircon[i].castShadow = true; 
			x.aircon[i].receiveShadow = false; 			
			
			x.acShadow[i].castShadow = false; 
			x.acShadow[i].receiveShadow = true; 
		
			grups[0].add( x.aircon[i] );		
			grups[0].add( x.acShadow[i] );		
		}
	...
	/*	for ( let i = 0; i < meshCount; i++ ) {	
			//if (i != 2) {
				...
				//matr[i] = new THREE.MeshBasicMaterial( { color: kolors[i], transparent: true, opacity: .7 } );
			//} else {
				matr[i] = new THREE.MeshStandardMaterial( { color: kolors[i], roughness: .5, metalness: .1 } );
			//}
				
			x.aircon.children[i].material = matr[i]; 		
		}	*/
		
		//x.aircon.children[0].material.envMap = x.skybox; 
		...
		//x.airconShadow = new THREE.Mesh( object.children[2].geometry, tempmat ); 
		//x.airconShadow.castShadow = false; 
		//x.airconShadow.receiveShadow = true; 
		
		//x.aircon.scale.set(.7, .7, .7); 
		
		//x.winShadow.scale.set(100, 100, 100); 
		//x.airconShadow.scale.set(100, 100, 100); 
		
		x.aircon[0].position.set(-40, florY+1.7, .5); 
		//x.aircon[0].rotation.set(0, 0, 0); 
		x.aircon[1].position.set(-15.5, florY+1.7, .5); 
		//x.aircon[1].rotation.set(0, 0, 0); 
		x.aircon[2].position.set(10, florY+1.7, .5); 
		//x.aircon[2].rotation.set(0, Math.PI/2, 0); 
		x.aircon[3].position.set(4, florY+19, -36.4); 
		x.aircon[3].rotation.set(0, Math.PI/-2, 0); 		
		
		x.acShadow[0].position.set(-39.9, florY+1.8, .6); 
		x.acShadow[1].position.set(-15.4, florY+1.8, .6); 
		x.acShadow[2].position.set(10.1, florY+1.8, .6); 
		x.acShadow[3].position.set(3.9, florY+19.1, -36.3); 
		x.acShadow[3].rotation.set(0, Math.PI/-2, 0); 
		
		//x.winShadow.position.set(.1, florY+.1, .1); 
		//x.winShadow.rotation.set(0, Math.PI/2, 0); 
		//x.airconShadow.position.set(.1, florY+.1, .1); 
		//x.airconShadow.rotation.set(0, Math.PI/2, 0); 
		
		//grups[0].add( x.aircon );		
		//grups[0].add( x.winShadow );		
		//grups[0].add( x.airconShadow );		
		
		
		let load1 = new THREE.TextureLoader(),  
			load2 = new THREE.TextureLoader(); 
			
		load1.load( 'obj/aircon/mat/diffuse1.jpg', function(tx) { 
			for ( let j = 0; j < 4; j++ ) {	
				x.aircon[j].material.map = tx; 
				//x.blinds.material.emissiveMap = tx; 
				//x.car.material.alphaMap = tx; 
				
				x.aircon[j].material.needsUpdate = true; 
			}
		}); 				

		load2.load( 'obj/aircon/mat/normal1.jpg', function(tx2) { 
			for ( let k = 0; k < 4; k++ ) {	
				x.aircon[k].material.normalMap = tx2; 
				
				x.aircon[k].material.needsUpdate = true; 
			}
		}); 				
...
function addTree() {
	...
	loader.load( 'obj/palmtree/palmtree.obj', function ( object ) {
		//console.log(object);
		...
		const scl = .5,
			rotY = [Math.PI/2, 0, Math.PI/-3], 
			pozX = [-48, -8, 32], 
			pozY = 0, 
			pozZ = -60; 
		
		matr0 = new THREE.MeshStandardMaterial( { color: kolors[0], roughness: .7, metalness: .1 } );
		matr1 = new THREE.MeshStandardMaterial( { color: kolors[0], roughness: .3, transparent: true, side: THREE.DoubleSide } );
		matr2 = new THREE.MeshStandardMaterial( { color: kolors[0], roughness: .5, metalness: .1 } );

		matr1.depthWrite = false; 
		
		let tempmat = new THREE.ShadowMaterial( { color: 0x111111, transparent: true, opacity: .3 } );	
		
		for ( let i = 0; i < 3; i++ ) {	
			//x.trunk[i] = object; 
		
			x.leaf[i] = new THREE.Mesh( object.children[1].geometry, matr1 ); 
		//	x.leafShadow[i] = new THREE.Mesh( object.children[1].geometry, tempmat ); 
			
			x.branch[i] = new THREE.Mesh( object.children[2].geometry, matr2 ); 
			x.branchShadow[i] = new THREE.Mesh( object.children[2].geometry, tempmat ); 
			
			x.trunk[i] = new THREE.Mesh( object.children[0].geometry, matr0 ); 
			x.trunkShadow[i] = new THREE.Mesh( object.children[0].geometry, tempmat ); 
			
			x.leaf[i].castShadow = x.branch[i].castShadow = x.trunk[i].castShadow = x.leaf[i].receiveShadow = true; 
			x.branch[i].receiveShadow = x.trunk[i].receiveShadow = false; 			
			
		//	x.leafShadow[i].castShadow = 
			x.branchShadow[i].castShadow = x.trunkShadow[i].castShadow = false; 
		//	x.leafShadow[i].receiveShadow = 
			x.branchShadow[i].receiveShadow = x.trunkShadow[i].receiveShadow = true; 
		
			x.leaf[i].scale.set(scl, scl, scl); 
			x.branch[i].scale.set(scl, scl, scl); 
			x.trunk[i].scale.set(scl, scl, scl); 

		//	x.leafShadow[i].scale.set(scl, scl, scl); 
			x.branchShadow[i].scale.set(scl, scl, scl); 
			x.trunkShadow[i].scale.set(scl, scl, scl); 
			
		//	x.leaf[i].rotation.y = x.branch[i].rotation.y = x.trunk[i].rotation.y = x.leafShadow[i].rotation.y = x.branchShadow[i].rotation.y = x.trunkShadow[i].rotation.y = rotY[i]; 
			x.leaf[i].rotation.y = x.branch[i].rotation.y = x.trunk[i].rotation.y = x.branchShadow[i].rotation.y = x.trunkShadow[i].rotation.y = rotY[i]; 
			
			x.leaf[i].position.set(pozX[i], florY+pozY, pozZ); 
			x.branch[i].position.set(pozX[i], florY+pozY, pozZ); 
			x.trunk[i].position.set(pozX[i], florY+0, pozZ); 
			
		//	x.leafShadow[i].position.set(pozX[i]+.1, florY+.1, pozZ+.1); 
			x.branchShadow[i].position.set(pozX[i]+.1, florY+pozY+.1, pozZ+.1);
			x.trunkShadow[i].position.set(pozX[i]+.1, florY+.1, pozZ+.1);
			
			grups[0].add( x.leaf[i] );		
			grups[0].add( x.branch[i] );		
			grups[0].add( x.trunk[i] );		
		//	grups[0].add( x.leafShadow[i] );		
			grups[0].add( x.branchShadow[i] );		
			grups[0].add( x.trunkShadow[i] );		
		}
		...
		load1.load( 'obj/palmtree/mat/barkd1.jpg', function(tx) { 
			for ( let j = 0; j < 3; j++ ) {	
				x.trunk[j].material.map = tx; 
				
				x.trunk[j].material.needsUpdate = true; 
			}
		}); 				

		load2.load( 'obj/palmtree/mat/leafd1.jpg', function(tx2) { 
			for ( let k = 0; k < 3; k++ ) {	
				x.leaf[k].material.map = tx2; 
				
				x.leaf[k].material.needsUpdate = true; 
			}
		}); 				

		load3.load( 'obj/palmtree/mat/branchd1.jpg', function(tx3) { 
			for ( let l = 0; l < 3; l++ ) {	
				x.branch[l].material.map = tx3; 
				
				x.branch[l].material.needsUpdate = true; 
			}
		}); 		
		
		load4.load( 'obj/palmtree/mat/leafo1.jpg', function(tx4) { 
			for ( let m = 0; m < 3; m++ ) {	
				x.leaf[m].material.alphaMap = tx4; 
				
				x.leaf[m].material.needsUpdate = true; 
			}
		}); 
		
		load5.load( 'obj/palmtree/mat/leafn1.jpg', function(tx5) { 
			for ( let n = 0; n < 3; n++ ) {	
				x.leaf[n].material.normalMap = tx5; 
				
				x.leaf[n].material.needsUpdate = true; 
			}
		}); 

		load6.load( 'obj/palmtree/mat/barkn1.jpg', function(tx6) { 
			for ( let o = 0; o < 3; o++ ) {	
				x.trunk[o].material.normalMap = tx6; 
				
				x.trunk[o].material.needsUpdate = true; 
			}
		}); 
...
function addChair() {
	...
	loader.load( 'obj/vaporw/loungechair.obj', function ( object ) {
		//console.log(object);
		...
		const scl = 7,
			rotX = Math.PI/-2, 
			rotZ = [Math.PI/-2, Math.PI/-2, Math.PI/2, Math.PI/2, 0, 0], 
			pozX = [19, 19, -53, -53, -24, -34], 
			pozY = [1.5, 1.5, 1.5, 1.5, 19.2, 19.2], 
			pozZ = [11, 18, 6, 13, -27, -27]; 
		
		matr0 = new THREE.MeshStandardMaterial( { color: kolors[0], roughness: .4, metalness: 0 } );
		let tempmat = new THREE.ShadowMaterial( { color: 0x111111, transparent: true, opacity: .3 } );	
		
		for ( let i = 0; i < 6; i++ ) {	
			x.chair[i] = new THREE.Mesh( object.children[0].geometry, matr0 ); 
			x.chairShadow[i] = new THREE.Mesh( object.children[0].geometry, tempmat ); 
			
			x.chair[i].castShadow = true; 
			x.chair[i].receiveShadow = false; 			
			
			x.chairShadow[i].castShadow = false; 
			x.chairShadow[i].receiveShadow = true; 
		
			x.chair[i].scale.set(scl, scl, scl); 
			x.chairShadow[i].scale.set(scl, scl, scl); 
			
			x.chair[i].rotation.x = x.chairShadow[i].rotation.x = rotX; 
			x.chair[i].rotation.z = x.chairShadow[i].rotation.z = rotZ[i]; 
			
			x.chair[i].position.set(pozX[i], florY+pozY[i], pozZ[i]); 
			x.chairShadow[i].position.set(pozX[i]+.1, florY+pozY[i]+.1, pozZ[i]+.1);
			
			grups[0].add( x.chair[i] );		
			grups[0].add( x.chairShadow[i] );		
		}
...
function addCar() {
	...
	loader.load( 'obj/vaporw/car2.obj', function ( object ) {
		//console.log(object);
		...
		for ( let i = 0; i < meshCount; i++ ) {	
			matr[i] = new THREE.MeshStandardMaterial( { color: kolors[i], roughness: .1, metalness: 0.1 } );
	...
	*/	
		...
		//matr[0].side = THREE.BackSide; 
		
		x.car = new THREE.Mesh( object.children[0].geometry, matr[0] ); 
		//x.car3 = new THREE.Mesh( object.children[0].geometry, tempmat ); 
		//console.log(x.car);
		
	//	x.car2 = new THREE.Mesh( object.children[0].geometry, matr[0] ); 
	...
	//	x.car2.position.set(0, florY-13, 50); 
		...
		//x.car.position.set(20, florY-.23, 70); 
		x.car.position.set(0, florY-.23, 70); 
		
		//x.car3.position.set(0, florY+13, 0); 
		
	//	x.car2.position.set(50, florY-13, 50); 
	//	x.car2.rotation.set(0, 0, Math.PI); 
	...
	//	grups[0].add( x.car2 );			
		...
		load1.load( 'obj/vaporw/mat/diffuse1.jpg', function(tx) { 
			//tx.wrapS = tx.wrapT = THREE.RepeatWrapping;    
			//tx.wrapS = tx.wrapT = THREE.MirroredRepeatWrapping;    
			//tx.repeat.set(4, 4);    		
			
			//for ( let j = 0; j < 1; j++ ) {	
				x.car.material.map = tx; 
				//x.car.material.alphaMap = tx; 
				
				x.car.material.needsUpdate = true; 
			//}
		}); 		
	}); 
...
//    (function walkIn3() {
	...
	//	if (win.ej[0]) grups[2].rotation.y = (z2 < 6.28) ? (z2 + .005) : 0; 
		...
		if (win.ej[0]) {
			//grups[1].rotation.y = (z2 < 1.57) ? (z2 + win.ej[2]) : (win.ej[2] *= -1); 
			
			if ((z2 > 1.9) || (z2 < -.8)) win.ej[2] *= -1; 
			
			grups[1].rotation.y = z2 + win.ej[2]; 
		}
	...
	//	if ( mixer ) mixer.update( delta );	
		...
		*/		
		...
		if (isMobil) {
			...
			camera.position.y = Math.cos(timer) * 20 + 20; 
			camera.position.z = Math.sin(timer) * 40 + 140; 
			...
			if (win.ej[0]) {
				...
				camera.position.y = (mouseY * -.04) + 20
			}
		}
...
function addAud() {
	...
	//	let url = 'analog-dreams-synthwave-9497'; 	
		//let url = 'loading-main-menu-145077'; 	
