...
//	onWindowResize(); 	
    ...
    (function fadeIn() {
		...
		if (!((val -= .05) < 0)) {
			...
			eL(document, 0, "scrollend", onScrollEnd); 
			eL(ui.btTop, 0, "click", clickbtTop); 
...
function imgSRCs() {
	...
	for ( let i = 0; i < aImgs.length; i++ ) {	
		...
		//console.log(aImgs[i].attributes.title.value); 
		//console.log(aImgs[i].children[0].attributes.alt.value); 
	}
...
//    if (event) event.preventDefault();
	...
	if (scrlY != window.pageYOffset) scrlY = window.pageYOffset; 
	
	_.skrolY = (scrlY < _.height) ? true : false; 
	
	//console.log(window.scrollY); 
	//console.log(_.skrolY); 
	//console.log(window.pageYOffset); 
	
	_.idleTimer = 0;
}	

function clickbtTop( event ) {
    if (event) event.preventDefault();
    //event.stopPropagation(); 
...
function animate() { 
	...
	if ((_.idleTimer < idleTO) && (_.skrolY)) {
	...
	//	mixer.update( delta );	
		...
		//console.log(ui.kontainer.scrollTop); 
