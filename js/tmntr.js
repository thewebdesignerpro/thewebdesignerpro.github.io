...
//if () {
	...
	/*if ( WebGL.isWebGL2Available() ) {
		...
		kontainer.style.background = "url('img/autumntreel.jpg') center top no-repeat"; 
...
function addGround() {
	const geometry = new THREE.SphereGeometry( 100, 32, 16, 0, Math.PI*2, 0, .8).toNonIndexed(); 
	const material = new THREE.MeshStandardMaterial( { color: 0xffffff, roughness: .9, metalness: .5 } ); 
	material.wireframe = true; 
	
	x.ground = new THREE.Mesh( geometry, material ); 
	
	//x.ground.position.y = -80; 
	x.ground.position.y = -59; 
	//x.ground.rotateX(Math.PI/-2); 
	//x.ground.castShadow = true; 
	x.ground.receiveShadow = true; 
	...
	const load0 = new THREE.TextureLoader(),  
		  ...
		  url2 = 'img/ground/moss/', 
		  frmt = '.jpg'; 
		
	load0.load( url2 + 'color0b' + frmt, function(tx0) { 
	...
	load1.load( url2 + 'rough0' + frmt, function(tx1) { 
	...
	load2.load( url2 + 'normal0' + frmt, function(tx2) { 
...
function addTree() {
	...
	loader.load( 'obj/' + url, function ( object ) {
		...
		x.tree = object; 
		//x.tree.receiveShadow = true; 
		
		const matr = [], 
			  url2 = 'obj/trees/13/tree2/', 
			  frm = 'jpg', 
			  kolor = [0xf3d4c5, 0xffffff], 
			  raf = [.75, .75], 
			  met = [0, .2];  
		
		for ( let i = 0; i < meshCount; i++ ) {	
		//for ( let i = 0; i < 1; i++ ) {	
			//x.tree.children[i].geometry.computeBoundingBox(); 
			...
			if (i==0) {
				...
				//x.tree.children[0].material = matr[i]; 
			} else {
				matr[i].alphaTest = .5; 
				matr[i].side = 2; 
				matr[i].shadowSide = 0; 
				x.leaves = new THREE.Mesh( object.children[0].children[0].geometry, matr[i] );
				//x.tree.children[0].children[0].material = matr[i]; 
			}
		}
		
	//	//x.tree.scale.set(.0008, .0008, .0008); 
	//	x.tree.position.set(0, 20, 0);
	//	//x.tree.position.set(-400, 0, -700);
	//	//x.tree.rotation.set(Math.PI/-2, 0, 0);
	//	x.tree.rotation.y = Math.PI/2 + .5; 
	//	scene.add(x.tree); 

		x.trunk.scale.set(7, 7, 7); 
		x.trunk.position.set(0, 20, 0);
		x.trunk.rotation.x = Math.PI/-2; 
		x.trunk.rotation.z = Math.PI/-1.75; 
		x.trunk.castShadow = true; 
		x.trunk.receiveShadow = true;		
		
		scene.add(x.trunk); 

		x.leaves.scale.set(7, 7, 7); 
		x.leaves.position.set(0, 20, 0);
		x.leaves.rotation.x = Math.PI/-2; 
		x.leaves.rotation.z = Math.PI/-1.75; 
		...
		//http://localhost/aistart/forest/obj/trees/13/1-130S01256334J.jpg
		...
		//loader0.load( url2 + 'bark_diffuse.jpg', function(tx0) { 	
		...
		//loader1.load( url2 + 'bark_normal.jpg', function(tx1) { 	
		loader1.load( url2 + 'normal0.' + frm, function(tx1) { 	
			tx1.wrapS = tx1.wrapT = THREE.RepeatWrapping;    
			//tx1.wrapS = tx1.wrapT = THREE.MirroredRepeatWrapping;    
		...
		loader2.load( url2 + 'color1.' + 'png', function(tx2) { 	
		...
		loader3.load( url2 + 'pink.' + 'png', function(tx3) { 	
			tx3.colorSpace = THREE.SRGBColorSpace;	
		
			matr[1].lightMapIntensity = 2; 
			matr[1].lightMap = tx3; 
			matr[1].needsUpdate = true; 
		}); 
...
function addLeaves() {
	...
	//const geom = new THREE.PlaneGeometry( 1, 2, 3, 5 );
	const geom = new THREE.PlaneGeometry( 1.5, 1.5, 5, 5 ); 
	//x.mater = new THREE.MeshStandardMaterial( { color: 0xdddddd, emissive: 0x505050, transparent: true, side: THREE.DoubleSide } );
	x.mater = new THREE.MeshStandardMaterial( { color: 0xdddddd, alphaTest: .5, side: THREE.DoubleSide } );
//	x.mater.depthWrite = false; 
	...
	x.Leaf.castShadow = true; 
		
	scene.add( x.Leaf ); 
		
	
	//load0.load( 'obj/trees/5/mat/leafc2.png', function(tx0) { 
	load0.load( 'obj/trees/13/tree2/dried.png', function(tx0) { 
		//for ( let j = 0; j < count; j++ ) {
			x.mater.map = tx0; 
			x.mater.needsUpdate = true; 
			x.mater.wireframe = false; 
...
//	}); 		
	...
	//load2.load( 'obj/trees/13/tree2/driedd.jpg', function(tx2) { 
		//for ( let l = 0; l < count; l++ ) {
			x.mater.displacementScale = .5; 
...
function randomizeMatrix2( matrix, i ) {
	const position = new THREE.Vector3();
	const rotation = new THREE.Euler();
	const quaternion = new THREE.Quaternion();
	const scale = new THREE.Vector3();			
...
function addGroundLeaves() {
	const matrix = new THREE.Matrix4(); 
	
	const geom = new THREE.PlaneGeometry( 2, 2, 5, 5 );

	const mater = new THREE.MeshStandardMaterial( { color: 0xeeeeee, alphaTest: .5, side: 2 } );

	mater.wireframe = true; 
	
	x.gLeaves = new THREE.InstancedMesh( geom, mater, count );
	x.gLeaves.instanceMatrix.setUsage( THREE.DynamicDrawUsage ); 
		
	for ( let i = 0; i < count; i++ ) {
		randomizeMatrix2( matrix, i ); 
		x.gLeaves.setMatrixAt( i, matrix ); 		
	}
	
	x.gLeaves.position.y = -59; 
	x.gLeaves.scale.y = .8; 
	x.gLeaves.castShadow = true; 
	x.gLeaves.receiveShadow = true; 
	...
	load0.load( 'obj/trees/13/tree2/orange.png', function(tx0) { 
		mater.map = tx0; 
		mater.needsUpdate = true; 
		mater.wireframe = false; 
	}); 		
	
	load1.load( 'obj/trees/5/mat/leafd.png', function(tx1) { 
		mater.displacementScale = .75; 
		mater.displacementMap = tx1; 
		mater.needsUpdate = true; 
	}); 		
...
function resample() {
	const surfaceGeometry = new THREE.SphereGeometry( 100, 24, 12, 0, Math.PI*2, 0, .5).toNonIndexed(); 
	const surfaceMaterial = new THREE.MeshLambertMaterial( { color: 0xffffff, wireframe: false } );
	const surface = new THREE.Mesh( surfaceGeometry, surfaceMaterial );
	surface.position.y = -59;
	surface.scale.y = .8;
	//surface.visible = false; 
	//scene.add(surface); 
	
//	//const vertexCount = x.ground.geometry.getAttribute( 'position' ).count;
...
//	console.timeEnd( '.sample()' );
	...
	x.gLeaves.instanceMatrix.needsUpdate = true;
...
function resampleParticle( i ) {
	...
	dummyL.position.copy( _position );
	//dummyL.scale.set( scales[ i ], scales[ i ], scales[ i ] );
	dummyL.scale.set( 1, 1, 1 );
	dummyL.lookAt( _normal );
	dummyL.updateMatrix();

	x.gLeaves.setMatrixAt( i, dummyL.matrix );

}
			
function addButterflies() {
	const kolor = [0x0000ff, 0x72ff00, 0xff0000, 0x00d8ff, 0xff9600, 0x9c00ff, 0xff0000, 0xff00b4], 
		  posX = [0, -10, 0, 10, 5, 5, -5, -5], 
		  posZ = [10, 0, -10, 0, 5, -5, -5, 5], 
		  rotZ = [0, Math.PI/-2, Math.PI, Math.PI/2, 
				  Math.PI/4 + Math.PI, Math.PI/1.5 + Math.PI, Math.PI/4, Math.PI/1.5]; 
	...
	for ( let i = 0; i < 8; i++ ) {	
		...
		const mater = new THREE.MeshStandardMaterial( { color: kolor[i], alphaTest: .5, side: 2 } );
		mater.wireframe = true; 
		
		x.butterflies[i] = new THREE.Mesh( geom, mater );

		x.butterflies[i].position.y = Math.random() * 20 + 23; 
		//x.butterflies[i].position.y = 23; 
		
		const mXZ = x.butterflies[i].position.y * .1; 
		//console.log(mXZ);		
		
		x.butterflies[i].position.x = posX[i] * mXZ; 
		x.butterflies[i].position.z = posZ[i] * mXZ; 		

		x.butterflies[i].rotation.x = Math.PI/-2; 	
		x.butterflies[i].rotation.z = rotZ[i]; 
		
		x.butterflies[i].castShadow = true; 
		x.butterflies[i].receiveShadow = true; 
			
		if (i<4) {
			grups[0].add( x.butterflies[i] ); 
		} else {
			//x.butterflies[i].rotation.z += Math.PI; 			
			grups[1].add( x.butterflies[i] ); 
		}
	}	
	...
	load0.load( 'img/butterfly.png', function(tx0) { 
		for ( let j = 0; j < 8; j++ ) {
		x.butterflies[j].material.map = tx0; 
		x.butterflies[j].material.needsUpdate = true; 
		x.butterflies[j].material.wireframe = false; 
		}
		
		animCat1(); 
	}); 		
	
	load1.load( 'img/butterflyd2.png', function(tx1) { 
		for ( let k = 0; k < 8; k++ ) {
		//x.butterflies[k].material.displacementScale = Math.random(); 
		x.butterflies[k].material.displacementMap = tx1; 
		x.butterflies[k].material.needsUpdate = true; 
		}
	}); 			
	
	load2.load( 'img/butterflya.png', function(tx2) { 
		for ( let l = 0; l < 8; l++ ) {
		x.butterflies[l].material.alphaMap = tx2; 
		x.butterflies[l].material.needsUpdate = true; 
		}
	}); 			
...
function animate() { 
 ...
 //   requestAnimationFrame(animate);
	...
	//	animWalk(); 
		...
		if (isMobil) {
			const rtY2 = Math.sin(timer/50) * Math.PI; 
				  
			camera.position.y = Math.sin(mSin5) * 16 + _.ej[5]; 
			...
			camera.position.y = _.pointer.y * 16 + _.ej[5]; 
		...
		x.leaves.rotation.set(Math.sin(timer*.8) * .01 + (Math.PI/-2), Math.sin(timer*1.3) * .01, Math.cos(timer*.8) * .01 + (Math.PI/-1.75)); 
		x.leaves.position.y = Math.cos(timer*.8) * .1 + 20; 
		...
		for ( let i = 0; i < 12; i++ ) {
			...
			let	lifX = x.dummy[i].position.x,  
				lifY = x.dummy[i].position.y - (Math.random() * .1 + .01), 
		...
		//	lifY = (lifY > 0.3) ? (x.dummy[i].position.y - 1) : 0.3; 
			...
			if (lifY < 10) {
				//lifY = x.dummy[i].position.x + .7; 
			//} else {
				lifX = Math.random() * 60 - 30; 
				lifY = Math.random() * 35 + 40; 
				lifZ = Math.random() * 60 - 30;  				
			}
		...
		//console.log(Math.sin(timer*1)); 
		
		//x.butterflies[0].material.displacementScale = Math.sin(timer*1); 
		
		const bfD = Math.sin(timer*20) * 1.7, 
			  bRotXZ = Math.sin(timer*1) * .1, 
		      bRotY = (grups[1].rotation.y + .005) % (Math.PI*2); 
			  
		//grups[0].rotation.y = -bRotY; 
		//grups[1].rotation.y = bRotY; 
		
		grups[0].rotation.set(bRotXZ, -bRotY, -bRotXZ); 
		grups[1].rotation.set(-bRotXZ, bRotY, bRotXZ); 
		
		for ( let j = 0; j < 8; j++ ) {	
			x.butterflies[j].material.displacementScale = (j % 2 == 0) ? bfD : -bfD; 
			//x.butterflies[j].material.displacementScale = bfD; 
				
			//x.butterflies[j].position.y = bfD + 28; 
			//x.butterflies[j].material.needsUpdate = true; 
		}		
