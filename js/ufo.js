...
if ( WebGL.isWebGL2Available() ) {
	...
	kontainer.style.background = "url('img/ufol.jpg') center top no-repeat"; 
...
//	scene.fog = new THREE.FogExp2(fogCol, 0.012);	
	...
	scene.add( new THREE.AmbientLight( 0xa1a1a1 ) );		

	x.spotLcone = []; 
	
	x.spotLight = []; 
	
	x.spotLight[0] = new THREE.SpotLight( 0xffffff, 3500, 90, Math.PI/6, 1 );
//	x.spotLight[0].position.set( 0, 22, 0 );
	x.spotLight[0].position.set( 0, 2, 0 );
	...
	x.spotLight[0].shadow.camera.far = 90;
...
function addMountain() {
	...
	loader.load( 'obj/mountain/2/m1080.obj', function ( object ) {
		//console.log(object);
		...
		for ( let i = 0; i < meshCount; i++ ) {	
			...
			matr[i] = new THREE.MeshStandardMaterial( { color: 0xffffff, roughness: .99, metalness: 0 } );
			matr[i].wireframe = true; 
			//matr[i].shadowSide = 2; 
			
			x.mountain[i] = new THREE.Mesh( object.children[i].geometry, matr[i] ); 
			//x.mountain[i].scale.set(3, 3, 3); 
			x.mountain[i].position.set(0, -50, 0);
			//x.mountain[i].rotation.set(Math.PI/-2, 0, Math.random() * Math.PI*2);			
			...
			//x.mountain.children[i].material = matr[i]; 
		...
		loader3.load( url2 + 'color1.jpg', function(tx3) { 	
			matr[1].map = tx3; 
		...
		loader4.load( url2 + 'normal1.jpg', function(tx4) { 	
			matr[1].normalMap = tx4; 
			matr[1].needsUpdate = true; 
		});  
		
		loader5.load( url2 + 'alfa1.jpg', function(tx5) { 	
			matr[1].alphaMap = tx5; 
			matr[1].needsUpdate = true; 
		});  
		
		loader6.load( url2 + 'metal0.jpg', function(tx6) { 	
			matr[0].metalnessMap = tx6; 
			matr[0].needsUpdate = true; 
		});  
...
function addUFO() {
	...
	loader.load( 'obj/ufo/0/ufo.obj', function ( object ) {
		//console.log(object);
		...
		for ( let i = 0; i < meshCount; i++ ) {	
			...
			matr[i] = new THREE.MeshStandardMaterial( { color: 0xffffff, roughness: .9, metalness: .5 } );
			...
			x.ufo.children[i].material = matr[i]; 
		}
		
		x.ufo.scale.set(11, 11, 11); 
		//x.ufo.scale.set(6.5, 6.5, 6.5); 
		//x.ufo.position.set(0, 20, 0);
		//x.ufo.rotation.set(Math.PI/-2, 0, 0);
		
		grups[0].add(x.ufo); 
		
		x.spotLight[1] = new THREE.SpotLight( 0xffffff, 3000, 25, Math.PI/4.9, 1 );
		x.spotLight[1].position.set( 0, -15, 0 );
		
		x.spotLight[2] = new THREE.SpotLight( 0xffffff, 2000, 25, Math.PI/5.5, 1 );
		x.spotLight[2].position.set( 0, 17, 0 );
	
		grups[0].add( x.spotLight[1] );	
		grups[0].add( x.spotLight[2] );	
		
		x.target1 = new THREE.Object3D(); 
		//x.target1.position.set(0, -8, 0); 
	...
	//	const spotLgeom = new THREE.ConeGeometry( 7, 30, 30, 1, true); 
	//	const spotLmater = new THREE.MeshBasicMaterial( {color: 0xfffbf2, side: 2, transparent: true, opacity: .2} ); 
	//	spotLmater.depthWrite = false; 
	//	spotLmater.fog = false; 	
	//	
	//	x.spotLcone[0] = new THREE.Mesh(spotLgeom, spotLmater ); 
	//	x.spotLcone[0].position.set( 0, -9, 0 );
	//	x.spotLcone[0].visible = false;	
	//	
	//	grups[0].add( x.spotLcone[0] );
		
		const spotLgeom = new THREE.PlaneGeometry( 24, 24 );		
		
		const spotLmater = new THREE.MeshBasicMaterial( {color: 0xfffdf9, transparent: true, opacity: .25, side: 2 } );
		spotLmater.depthWrite = false; 
		//spotLmater.fog = false; 		
		...
		for ( let j = 0; j < 8; j++ ) {	
			...
			x.spotLcone[j].position.set( 0, -11, 0 );
			x.spotLcone[j].rotateY(rotY[j]);
			//x.spotLcone[j].scale.set( 2, 2, 1 );
			x.spotLcone[j].visible = false;			
			
			grups[0].add( x.spotLcone[j] );	
	    }
		
		//loader5.load( url2 + 'cone0b.jpg', function(tx5) { 	
		loader5.load( 'img/spotLconeA.png', function(tx5) { 	
			spotLmater.alphaMap = tx5; 
...
function animate() { 
	...
	if (_.idleTimer < idleTO) {
		...
		if (!x.rotCam) {
			if (isMobil) {
				camera.position.y = Math.sin(mSin5) * 38; 
				...
				camera.position.y = _.pointer.y * 38; 
