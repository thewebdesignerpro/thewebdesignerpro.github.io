...
if ( WebGL.isWebGL2Available() ) {
	...
	kontainer.style.background = "url('img/islandl.jpg') center top no-repeat"; 
...
//	scene.fog = new THREE.FogExp2(fogCol, 0.0002);	
	//scene.fog = new THREE.FogExp2(0xff0000, 0.0002);	
...
//	x.spotLcone = []; 
	...
	x.spotLight[0] = new THREE.SpotLight( 0xfff0dd, 28000000, 3000, Math.PI/16, 1 );
	//x.spotLight[0] = new THREE.SpotLight( 0xffe5aa, 50000000, 7000, Math.PI/8, 1 );
	x.spotLight[0].position.set( 0, 2000, 0 );
...
//	x.spotLight[0].shadow.mapSize.height = 256;
	...
	x.spotLight[0].shadow.intensity = .82;
...
function addSea() {
	...
	x.sea = new Water(
		waterGeometry,
		{
			textureWidth: 1280,
			textureHeight: 1280,
			//waterNormals: new THREE.TextureLoader().load( 'img/water/test1.jpg', function ( texture ) {
...
function addIsland() {
	...
	loader.load( 'obj/island/0/slnd.obj', function ( object ) {
		//console.log(object);
		...
		const matr = [], 
			  url2 = 'obj/island/0/mat/', 
			  frm = 'jpg'; 
		
		for ( let i = 0; i < meshCount; i++ ) {	
			//x.char1.children[i].geometry.computeBoundingBox(); 
			
			matr[i] = new THREE.MeshStandardMaterial( { color: 0xffffff, roughness: 1, metalness: 1 } );
			matr[i].alphaTest = .5; 
			matr[i].shadowSide = 1; 
			
			x.island.children[i].material = matr[i]; 
		}
		
		grups[0].position.set(0, .4, 0); 
		grups[0].rotation.set(0, Math.random() * Math.PI, 0); 
		grups[0].add(x.island); 
		
		const geometry = new THREE.PlaneGeometry( 680, 680 );
		const material = new THREE.MeshStandardMaterial( {color: 0xffffff, transparent: true, opacity: .8} );
		//material.alphaTest = .5; 
		material.depthWrite = false; 
		//material.blending = THREE.AdditiveBlending; 
		
		x.islandEdge = new THREE.Mesh( geometry, material );
		x.islandEdge.rotation.set(Math.PI/-2, 0, Math.PI); 
		x.islandEdge.position.y = .2; 
		x.islandEdge.receiveShadow = true; 
		grups[0].add( x.islandEdge );		
		
		scene.add(grups[0]); 

		x.spotLight[0].target = x.island; 
		...
		loader1.load( url2 + 'color1.jpg', function(tx1) { 	
			x.island.children[0].material.map = tx1; 
			x.island.children[0].material.needsUpdate = true; 
			
			//fadeScene(); 
		});  
		
		loader2.load( url2 + 'rough1.jpg', function(tx2) { 	
			x.island.children[0].material.roughnessMap = tx2; 
			x.island.children[0].material.needsUpdate = true;
		});  
		
		loader3.load( url2 + 'metal1.jpg', function(tx3) { 	
			x.island.children[0].material.metalnessMap = tx3; 
			x.island.children[0].material.needsUpdate = true;
		});  
			
		loader4.load( url2 + 'normal1.jpg', function(tx4) { 	
			x.island.children[0].material.normalMap = tx4; 
			x.island.children[0].material.needsUpdate = true;
		});  
		
		loader5.load( url2 + 'alfa1.jpg', function(tx5) { 	
			x.island.children[0].material.alphaMap = tx5; 
			x.island.children[0].material.needsUpdate = true;
		});  

		loader6.load( url2 + 'color0.jpg', function(tx6) { 	
			x.islandEdge.material.map = tx6; 
			x.islandEdge.material.needsUpdate = true;
		});  
		
		loader7.load( url2 + 'alfa0.jpg', function(tx7) { 	
			x.islandEdge.material.alphaMap = tx7; 
			x.islandEdge.material.needsUpdate = true;
		});  
...
function addBoat() {
	...
	loader.load( 'obj/boat/0/boat.obj', function ( object ) {
		...
		x.boat.scale.set(28, 28, 28); 
		x.boat.position.set(107, 7, 65);
		x.boat.rotation.set(.1, Math.PI - .3, .05);
		grups[0].add(x.boat); 
...
function animate() { 
	...
	if (_.idleTimer < idleTO) {
		...
		x.sea.material.uniforms[ 'time' ].value -= .003; 
		
		x.boat.rotation.x = Math.cos(timer) * .05 + .1; 
...
function animFBX() {
	...
	//	x.char1.rotation.set(0, _.ej[2], 0);	
		...
		x.char1.position.set(4, 2.7, -37); 
		x.char1.rotation.set(0, -2.2, 0);	
