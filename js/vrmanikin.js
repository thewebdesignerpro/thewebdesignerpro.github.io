...
var mouseX = mouseY = mouseZ = 0, ww = window.innerWidth, wh = window.innerHeight, wwh = ww/2, whh = wh/2, pis=0, pif=pif2=1, ntro = true, 
	renderer, camera, scene, controls, stats, timer, effect, vrDisplay;
...
/*	controls = new THREE.OrbitControls(camera, renderer.domElement);
	...
	if (WEBVR.isAvailable()===true) {
		...
		cntnr.appendChild(WEBVR.getButton(effect));
	}
...
function animate() {
	...
	//camera.position.z = Math.cos(timer*.1) * 100 + 20;
	render();
}
