...
if ( WebGL.isWebGL2Available() ) {
	...
	kontainer.style.background = "url('img/aliendancel.jpg') center top no-repeat"; 
...
//	scene.fog = new THREE.FogExp2(fogCol, 0.012);	
	...
	camera.position.set(0, _.ej[5], 75); 
	//camera.lookAt( 0, 0, 0 );
	...
	x.spotLight[0] = new THREE.SpotLight( 0xffffff, 120000, 120, Math.PI/10, 1 );
	x.spotLight[0].position.set( 0, 100, 0 );
...
function addStage() {
	...
	loader.load( 'obj/stage/7/stage.obj', function ( object ) {
		//console.log(object);
		...
		for ( let i = 0; i < meshCount; i++ ) {	
			...
			matr[i] = new THREE.MeshStandardMaterial( { color: 0xffffff, roughness: 0, metalness: .3 } );
			matr[i].emissive.setHex(0xffffff); 
			matr[i].emissiveIntensity = .75; 
			matr[i].wireframe = true; 
			//matr[i].shadowSide = 2; 
			
			matr[i].envMapRotation.y = Math.PI/2; 
			matr[i].envMapIntensity = envMI[i]; 
			matr[i].envMap = scene.background; 				
			
			x.stage[i] = new THREE.Mesh( object.children[i].geometry, matr[i] ); 
			//x.stage[i].scale.set(3, 3, 3); 
			x.stage[i].position.set(0, -10, 0);
			//x.stage[i].rotation.set(Math.PI/-2, 0, Math.random() * Math.PI*2);			
			x.stage[i].rotation.set(0, Math.PI/-2, 0);			
			
			x.stage[i].receiveShadow = true; 
		...
		loader1.load( url2 + 'color1.jpg', function(tx1) { 	
			matr[1].map = matr[1].emissiveMap = tx1; 
		...
		loader2.load( url2 + 'alfa1b.jpg', function(tx2) { 	
			matr[1].alphaMap = tx2; 
...
function addMirror() {
	...
	geometry[0] = new THREE.CircleGeometry( 18, 32 ); 
	geometry[1] = new THREE.RingGeometry( 15, 30.25, 32 ); 
	geometry[2] = new THREE.RingGeometry( 27, 44.25, 32 ); 
	
	x.mirror[0] = new Reflector( geometry[0], {
		clipBias: 0.003,
	...
	x.mirror[1] = new Reflector( geometry[1], {
		clipBias: 0.003,
		textureWidth: _.width * window.devicePixelRatio,
		textureHeight: _.height * window.devicePixelRatio,
		color: 0xb1b3b3
	...
	x.mirror[2] = new Reflector( geometry[2], {
		clipBias: 0.003,
		textureWidth: _.width * window.devicePixelRatio,
		textureHeight: _.height * window.devicePixelRatio,
		color: 0xb1b3b3
	...
	x.mirror[0].position.set(0, -10.2, 0);
	//x.mirror[0].position.set(0, -9.89, 0);
	x.mirror[1].position.set(0, -11.3, 0);
	x.mirror[2].position.set(0, -12.42, 0);
	x.mirror[0].rotation.x = x.mirror[1].rotation.x = x.mirror[2].rotation.x = Math.PI/-2;
	
	//grups[0].add( x.mirror[0] );	
	scene.add( x.mirror[0] );	
	scene.add( x.mirror[1] );	
	scene.add( x.mirror[2] );	
}
...
*/
		...
		if (x.sound) {
			...
			//	let data3 = (data/88); 
				...
				if (data3 > 0) { 
					x.stage[0].material.emissiveIntensity = x.stage[1].material.emissiveIntensity = data3; 
					x.spotLight[0].intensity = data3 * 120000; 
					...
					x.stage[0].material.emissiveIntensity = x.stage[1].material.emissiveIntensity = 0; 
					x.spotLight[0].intensity = 0; 
				...
				x.stage[0].material.emissiveIntensity = x.stage[1].material.emissiveIntensity = .75; 
				x.spotLight[0].intensity = 120000; 
...
function animAlien() {
	...
	loader.load( 'obj/' + url, function ( object ) {
		...
		x.alien.scale.set(5.5, 5.5, 5.5); 
		x.alien.position.set(0, -10, 0);
