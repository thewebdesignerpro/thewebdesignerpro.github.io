...
var light = new THREE.PointLight( 0xffffff, 13, 2000 );
light.position.set( 0, 6, -32 );
scene.add( light );

var spotL = new THREE.SpotLight( 0xffffff, 1, 1000 );
spotL.position.set( 0, 6, -32 );
...
loada.load(
	'imj/wves.jpg',
	function ( tex ) {
		mater = new THREE.MeshPhongMaterial( { map: tex, color: 0x324855, specular: 0x070707, shininess: 1, side: THREE.FrontSide, wrapAround: true, overdraw: 0.5 } );
...
loada.load(
	'imj/arw2.png',
	function ( tex2 ) {
		var geom2 = new THREE.PlaneGeometry( 70, 70 );
		mater = new THREE.MeshLambertMaterial( { map: tex2, side: THREE.FrontSide, opacity: 0.8, transparent: true } );
		var arw = new THREE.Mesh( geom2, mater );
		arw.position.set(0, 5, -40);
		arw.castShadow = false;
		arw.receiveShadow = false;
		scene.add( arw );		
		geom2 = new THREE.PlaneGeometry( 100, 100 );
		mater = new THREE.MeshLambertMaterial( { map: tex2, side: THREE.FrontSide, opacity: 0.5, transparent: true } );
		var arw2 = new THREE.Mesh( geom2, mater );
		arw2.position.set(0, 5, -40);
...
mater = new THREE.MeshBasicMaterial( { color: 0xfffdfb, side: THREE.FrontSide } );
var sun = new THREE.Mesh( new THREE.CircleGeometry( 2, 16 ), mater );
sun.position.set(0, 5, -40);
