...
if ( WebGL.isWebGL2Available() ) {
	...
	kontainer.style.background = "url('img/wandererl.jpg') center top no-repeat"; 
...
//	ui.swtchKam.style.visibility = "hidden"; 
   ...
   if (window.getComputedStyle(dummy, null).getPropertyValue("left")=='9000px') {
    ...
    ui.kontainer.style.backgroundColor = '#b0b4b7';		
	...
	_.ej[3] = 3000; 			//2000 or 0 - camGrup pos z
	...
	scene.add( new THREE.AmbientLight( 0xc2c2c2 ) );		
...
//	x.spotLight[0].shadow.camera.fov = 40;
	...
	x.directionalLight = new THREE.DirectionalLight( 0xffffff, 3.5 );
	//const directionalLight = new THREE.DirectionalLight( 0xffffff, 4 );
	x.directionalLight.castShadow = true; 
	//directionalLight.shadow.mapSize.width = 1024; 
	//directionalLight.shadow.mapSize.height = 1024; 
	//directionalLight.shadow.camera.near = 1; 
	x.directionalLight.shadow.camera.far = 7000;	
	...
	x.directionalLight.position.set( 0, 3000, -3000 );
...
//	grups[0].add(camera);	
	...
	//x.target0.position.set(0, _.ej[5], -500); 
...
function addClouds() {
	...
	const geometry = new THREE.SphereGeometry( 2900, 32, 16, 0, Math.PI*2, 0, Math.PI/2 ); 
...
function addRoads() {
	...
	//for ( let i = 0; i < 4; i++ ) {	
	for ( let i = 0; i < 1; i++ ) {	
		roadMater[i] = new THREE.MeshStandardMaterial( { color: kolor, roughness: .9, metalness: .1 } ); 
	...
	loader0.load( url + 'color012.jpg', function(tx0) { 	
	...
	loader0b.load( url + 'rough012.jpg', function(tx0b) { 	
	...
	loader0c.load( url + 'normal012.jpg', function(tx0c) { 	
		tx0c.wrapS = tx0c.wrapT = THREE.RepeatWrapping;    
		tx0c.repeat.set(1, 2);  
		
		x.road[0].material.normalScale.set(3, 3); 
...
function addGround() {
	...
	//let geometry = new THREE.PlaneGeometry( width, height, rez, rez * 1.5 );
...
function randomizeMatrix( matrix ) {
	...
	position.z = (Math.random() * 2990 - 1495);
