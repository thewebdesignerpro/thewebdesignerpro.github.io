...
var mouseX = mouseY = mouseZ = 0, ww = window.innerWidth, wh = window.innerHeight, wwh = ww/2, whh = wh/2,	controls, entro = true,
	...
	geom, mater, x = y = z = pis = 0, pif = 1, ntro = true, bebe, bebe2, bebe3, bebe4, anima0, anima1, anima2, anima3, startT = Date.now(), oldT = 0, frame, animp, diamond, diamondR, diamondW;
...
//loada.load('imj/map/cube/skybx2.jpg', function(tex) {
...
//function bB() {				
	...
	loader.load('obj/diamond.json', function (geometry, materials) {
		//console.log('y');
		...
		mater = new THREE.MeshPhongMaterial({envMap: cubeMapR, overdraw: 0.5, side: THREE.DoubleSide, skinning: false, transparent: true, opacity: 0.3, shading: THREE.FlatShading, refractionRatio: 0.97});
