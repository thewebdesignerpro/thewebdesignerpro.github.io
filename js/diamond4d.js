...
var pointL2 = new THREE.PointLight( 0xffffff, 10, 100 );
pointL2.position.set( -14, 2, 14 );
pointL2.position.set( 1.8, 1.5, -1.8 );
...
//function bB() {				
	...
	loader.load('obj/diamond-round.json', function (geometry, materials) {
		...
		mater = new THREE.MeshPhongMaterial({envMap: cubeMap, overdraw: 0.5, side: THREE.DoubleSide, skinning: false, wireframe: false, transparent: true, opacity: 0.5, shading: THREE.FlatShading, shininess: 60, metal: false});
		...
		mater = new THREE.MeshPhongMaterial({envMap: cubeMapR, overdraw: 0.5, side: THREE.DoubleSide, skinning: false, transparent: true, opacity: 0.5, shading: THREE.FlatShading, refractionRatio: 0.98});
		...
		Gem.add( dglow );			
		var glowM1 = new THREEx.GeometricGlowMesh(dglow, 2.0, 0.05, 20.0);
