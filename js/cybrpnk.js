...
function addCar() {
	...
	loader.load( 'obj/outrun/lowpolycar.obj', function ( object ) {
		...
		testMatr[0] = new THREE.MeshStandardMaterial( { color: 0x5e5e5e, roughness: 0, metalness: .35, envMap: x.skybox, side : 2 } );	// main frame
		testMatr[1] = new THREE.MeshStandardMaterial( { color: 0x5e5e5e, roughness: 0, metalness: .35, envMap: x.skybox, side : 2 } );	// side trim
		testMatr[2] = new THREE.MeshBasicMaterial( { color: 0x474747 } );	// inner frame
		testMatr[3] = new THREE.MeshBasicMaterial( { color: 0x9eddfd, transparent:true, opacity: .9, envMap: x.skybox } );	// windows
		...
		testMatr[0].envMapIntensity = testMatr[1].envMapIntensity = 20; 
